...
/**
 ...
 表示客户端的类：
            java.net.Socket:此类实现客户端套接字。套接字是两台机器间通信的端口；
            套接字：包含了IP地址和端口的网络单位；
 ...
 * 2.使用Socket对象中的方法getOutputStream()获取网络字节输出；
...
public class TcpClient {
    public static void main(String[] args) throws IOException {
        ...
        //使用Socket对象中的方法getOutputStream()获取网络字节输出；
        OutputStream os=socket.getOutputStream();
        //使用网络字节输出流OutputStream对象中的方法write，给服务器发送数据；
        os.write("这是一条给唐圣昊的消息".getBytes());
        //使用Socket对象中的getInputStream()获取网络字节输入流InputStream对象；
        InputStream is=socket.getInputStream();
        //使用网络字节流InputStream对象中的方法read,读取服务器回写的数据；
